...
while [[ $# -gt 0 ]]; do
        case $1 in
                ...
                debug)
                        ...
                        ;;
                --clean)
                        CARGO_TARGET_DIR=$DATA_DIR
                        # Workaround for bevy assets dir not being found when changing the TARGET_DIR
                        BEVY_ASSET_PATH=$SRC_DIR/assets
                        export CARGO_TARGET_DIR
                        export BEVY_ASSET_PATH
                        build_dir=$DATA_DIR
                        shift
                        ;;
                -h|--help)
...
case $cargo_arg in
        build)
                cp $(find ${build_dir}/release -maxdepth 1 -not -path '*/.*' -type f) ${OUTPUT_DIR}
                ;;
esac
